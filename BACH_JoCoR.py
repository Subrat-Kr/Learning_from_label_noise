...
parser.add_argument('--w_decay', type=float, default=0.03)
...
parser.add_argument('--co_lambda', type=float, default=0.35)
...
class CNN(nn.Module):
    def __init__(self, input_channel=3, n_outputs=4, dropout_rate=0.25, momentum=0.1):
        ...
        self.linear1=nn.Linear(4096, n_outputs)    #modified from 256 to 87040 by gora then 5376 to 256 again
...
def loss_jocor(y_1, y_2, t, forget_rate, ind, noise_or_not, co_lambda=0.45, is_train = None):
   ...
   # print(num_remember)# //////////////////////////////// 4 feb
    ...
    pure_ratio = np.sum(noise_or_not[ind[ind_sorted[:num_remember]]])/int(num_remember)  # change int to float by SUBRAT 
    ...
    return loss, loss, pure_ratio, pure_ratio  #why twice the variable 'loss' is getting passed?? because they are same for the both model getting calculated from both of the model
...
transform=transforms.Compose([ 
        ...
        transforms.RandomRotation(degrees=50, interpolation = InterpolationMode.BILINEAR),
        transforms.Resize(180, interpolation=InterpolationMode.BILINEAR, max_size=None, antialias=None),
...
train_dataset = BACH(csv_file="/home/subrat/JoCoR_bach/Data/train_data_clean.csv",transform=transform,noise_type=args.noise_type) 
val_dataset = BACH(csv_file="/home/subrat/JoCoR_bach/Data/val_data_clean.csv",transform=transform,noise_type=args.noise_type)
...
def main():
    ...
    val_loader = DataLoader(dataset=val_dataset, batch_size=batch_size, num_workers=4, shuffle=True)
    ...
    val_acc1, val_acc2, _,_,_,_ = model.evaluate_1(val_loader, epoch)
    ...
    for epoch in range(1, args.n_epoch):
        ...
        print(  'Val   Loss: Model1 %.4f %% Model2 %.4f' % ( loss_1, loss_2))
        ...
        if pure_ratio_1_list is None or len(pure_ratio_1_list) == 0:
            ...
            print(
                'Train Loss: Model1 %.4f %% Model2 %.4f' % ( loss_1, loss_2))
